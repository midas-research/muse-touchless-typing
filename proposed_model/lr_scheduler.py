...
class AdjustLR(object):
    ...
    def step(self, epoch):
        """
        Step down learning rate for each epoch.
        new_lr = init_lr / 2^(epoch-sleep_epochs)/half)
        """
        
        if epoch >= self.sleep_epochs:
            for idx, param_group in enumerate(self.optimizer.param_groups):
                new_lr = self.init_lr[idx] * math.pow(0.5, (epoch-self.sleep_epochs)/float(self.half))
